...
//	user.do_attack_animation(src, no_effect = TRUE)
	...
	user.visible_message(span_danger("<b>[user]</b> hits [src] with [weapon][no_damage ? ", and it doesn't leave a scratch" : ""]!"), \
						span_danger("I hit [src] with [weapon][no_damage ? ", and it doesn't leave a scratch" : ""]!"), \
