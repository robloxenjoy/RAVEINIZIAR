...
/obj/item/stack/medical/suture/heal_carbon(mob/living/carbon/C, mob/user, brute, burn)
	...
	if(affecting.get_incision(FALSE))
		...
		if(affecting.is_artery_torn())
			...
			if(user.diceroll(GET_MOB_SKILL_VALUE(user, SKILL_SURGERY)+4, context = DICE_CONTEXT_PHYSICAL) <= DICE_CRIT_FAILURE)
				to_chat(user, span_warning(fail_msg()))
				return FALSE
		...
		if(affecting.is_tendon_torn())
			...
			if(user.diceroll(GET_MOB_SKILL_VALUE(user, SKILL_SURGERY)+5, context = DICE_CONTEXT_PHYSICAL) <= DICE_CRIT_FAILURE)
				to_chat(user, span_warning(fail_msg()))
				return FALSE
