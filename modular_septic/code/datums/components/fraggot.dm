...
//	for(var/mob/living/carbon/human/human in (GLOB.mob_living_list))
	SEND_SOUND(world, sound('modular_pod/sound/eff/kill_her_now_kill_her_now.ogg', 70))
	to_chat(world, "<span class='warning'><span class='big bold'>[emoji_parse(":fatal:")][our_fraggot.real_name], THE [our_fraggot.mind?.assigned_role.title], IS A FATAL! KILL THIS CREATURE![emoji_parse(":chaos:")]</span></span>")
...
//		our_fraggot.gib()
		...
		SEND_SOUND(world, sound('modular_pod/sound/eff/fataldeath.ogg', 70))
		for(var/mob/living/carbon/human/human in (GLOB.mob_living_list - our_fraggot))
			SEND_SIGNAL(human, COMSIG_CLEAR_MOOD_EVENT, "[our_fraggot.real_name]")
