...
/datum/hacking/ui_interact(mob/user, datum/tgui/ui)
	...
	if (!ui)
		ui = new(user, src, "Hacking", "[holder.name] Hacking")
		ui.open()
...
/datum/hacking/proc/start_hacking(hacking_action = "Destroy", mob/living/hackerman)
	...
	hacker_attack = GET_MOB_SKILL_VALUE(hackerman, SKILL_ELECTRONICS) * 7
	hacker_health = GET_MOB_SKILL_VALUE(hackerman, SKILL_ELECTRONICS) * 10
...
/datum/hacking/proc/hacker_attack(attack_type = "Attack")
	...
	switch(attack_type)
		if("Attack")
			...
			var/damage = max(0, hacker_attack + electronics_bonus - holder_defense)
...
/datum/hacking/proc/unset_holder(atom/previous_holder)
	holder = null
	UnregisterSignal(previous_holder, COMSIG_PARENT_QDELETING)
	stop_hacking()
