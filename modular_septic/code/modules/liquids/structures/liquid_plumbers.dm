...
/obj/machinery/plumbing/liquid_input_pump/default_unfasten_wrench(mob/user, obj/item/I, time)
	. = ..()
	if(. == SUCCESSFUL_UNFASTEN)
		turned_on = FALSE

/obj/machinery/plumbing/liquid_input_pump/attack_hand(mob/living/user, list/modifiers)
	. = ..()
	if(!anchored)
		to_chat(user, span_warning("[src] needs to be anchored first!"))
		return
	to_chat(user, span_notice("I turn [src] [turned_on ? "off" : "on"]."))
	turned_on = !turned_on

/obj/machinery/plumbing/liquid_input_pump/CtrlClick(mob/user)
	. = ..()
	if(anchored)
		return ..()
...
/obj/machinery/plumbing/liquid_input_pump/proc/can_pump()
	...
	if(height_regulator && turf_loc.liquids.liquid_height < height_regulator)
		return FALSE
...
/obj/machinery/plumbing/liquid_input_pump/process(delta_time)
	...
	var/target_value = delta_time * (drain_flat + (turf_loc.liquids.total_reagents * drain_percent))
	//Free space handling
	var/free_space = reagents.maximum_volume - reagents.total_volume
	if(target_value > free_space)
		target_value = free_space

	var/datum/reagents/temporary_holder = turf_loc.liquids.take_reagents_flat(target_value)
	temporary_holder.trans_to(src, temporary_holder.total_volume)
...
/obj/machinery/plumbing/liquid_output_pump/default_unfasten_wrench(mob/user, obj/item/I, time)
	. = ..()
	if(. == SUCCESSFUL_UNFASTEN)
		turned_on = FALSE

/obj/machinery/plumbing/liquid_output_pump/attack_hand(mob/living/user, list/modifiers)
	. = ..()
	if(!anchored)
		to_chat(user, span_warning("[src] needs to be anchored first!"))
		return
...
/obj/machinery/plumbing/liquid_output_pump/CtrlClick(mob/user)
	. = ..()
	if(anchored)
		return
	if(!user.canUseTopic(src, BE_CLOSE, NO_DEXTERITY, FALSE, TRUE))
...
/obj/machinery/plumbing/liquid_output_pump/process(delta_time)
	...
	turf_loc.add_liquid_from_reagents(tempr)
	qdel(tempr)
...
/obj/machinery/plumbing/liquid_output_pump/proc/can_pump()
	...
	if(height_regulator)
		var/turf/T = loc
		if(T.liquids && T.liquids.liquid_height >= height_regulator)
			return FALSE
	return TRUE
