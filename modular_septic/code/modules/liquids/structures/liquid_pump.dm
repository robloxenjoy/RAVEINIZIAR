...
/obj/structure/liquid_pump/wrench_act(mob/living/user, obj/item/tool)
	. = ..()
	default_unfasten_wrench(user, tool, 40)
	if(!anchored && turned_on)
		toggle_working()
	return TRUE

/obj/structure/liquid_pump/attack_hand(mob/living/user, list/modifiers)
	. = ..()
	if(!anchored)
...
/obj/structure/liquid_pump/AltClick(mob/living/user)
	if(!user.canUseTopic(src, BE_CLOSE, NO_DEXTERITY, FALSE, TRUE))
		return
	to_chat(user, span_notice("I flick [src]'s spewing mode [spewing_mode ? "off" : "on"]."))
...
/obj/structure/liquid_pump/examine(mob/user)
	...
	. += span_notice("It's mode currently is set to [spewing_mode ? "SPEWING" : "SIPHONING"].</span> (Alt-click to switch)")
...
/obj/structure/liquid_pump/process()
	...
	if(spewing_mode)
		...
		if(!free_space)
			return
		var/target_siphon_amt = (turf_loc.liquids.total_reagents * pump_speed_percentage) + pump_speed_flat
		if(target_siphon_amt > free_space)
			target_siphon_amt = free_space
		var/datum/reagents/tempr = turf_loc.liquids.take_reagents_flat(target_siphon_amt)
