	...
	switch(effective_blood_oxygenation)
		if(BLOOD_VOLUME_OKAY to BLOOD_VOLUME_SAFE)
			if(DT_PROB(2.5, delta_time))
				to_chat(src, span_warning("<i>I feel [pick("somewhat dizzy","vague","weak")].</i>"))
...
//			owner.blur_eyes(4)
			if(DT_PROB(5, delta_time))
				...
				to_chat(owner, span_userdanger("<i>I feel really [pick("vague","strange","weak")]...</i>"))
...
//			owner.blur_eyes(6)
			if(DT_PROB(10, delta_time))
				...
				to_chat(owner, span_userdanger("<i>I feel so [pick("crazy","unclear","terrible")]...</i>"))
...
//			owner.blur_eyes(6)
			if((brain.current_blood <= 0) && DT_PROB(7.5, delta_time))
				to_chat(owner, span_userdanger("<i>I feel so [pick("heavy", "empty", "bleak")] and [pick("pale", "weak")]!</i>"))
