...
//	if(!owner || (limb_integrity > 0) || !can_dismember() || (wounding_dmg < DISMEMBER_MINIMUM_DAMAGE) || (wound_bonus == CANT_WOUND))
	if(!owner || !can_dismember() || (limb_integrity > (max_damage/2.7)) || (wounding_dmg < DISMEMBER_MINIMUM_DAMAGE) || (wound_bonus == CANT_WOUND))
		return FALSE
...
*/
	...
	if((get_damage() + wounding_dmg) > limb_integrity)
		apply_dismember(wounding_type, TRUE, TRUE)
		return TRUE
...
/obj/item/bodypart/proc/apply_dismember(wounding_type = WOUND_SLASH, silent = FALSE, add_descriptive = TRUE)
	...
	if(add_descriptive)
		switch(wounding_type)
			if(WOUND_SLASH)
				SEND_SIGNAL(was_owner, COMSIG_CARBON_ADD_TO_WOUND_MESSAGE, span_bolddanger(" [span_big("<b><i>\ [name] sails off in arc!</i></b>")]"))
			if(WOUND_PIERCE, WOUND_BLUNT)
				SEND_SIGNAL(was_owner, COMSIG_CARBON_ADD_TO_WOUND_MESSAGE, span_bolddanger(" [span_big("<b><i>\ [name] destroys!</i></b>")]"))
			if(WOUND_BURN)
				SEND_SIGNAL(was_owner, COMSIG_CARBON_ADD_TO_WOUND_MESSAGE, span_bolddanger(" [span_big("<b><i>\ [name] burns!</i></b>")]"))
