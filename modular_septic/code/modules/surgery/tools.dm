...
/obj/item/shears/attack(mob/living/M, mob/living/user)
	...
	if(user.zone_selected == BODY_ZONE_PRECISE_GROIN)
		...
		if(!limb_snip_candidate)
			to_chat(user, span_warning("[patient] is already missing that limb, what more could I want?"))
			return
		candidate_name = limb_snip_candidate.name

	var/amputation_speed_mod = 1

	patient.visible_message(span_danger("[user] begins to secure [src] around [patient]'s [candidate_name]."), \
					span_userdanger("[user] begins to secure [src] around my [candidate_name]!"))
	...
	if(do_after(user,  toolspeed * 15 SECONDS * amputation_speed_mod, target = patient))
		...
		user.visible_message(span_danger("[src] violently slams shut, amputating [patient]'s [candidate_name]."), \
						span_notice("I amputate [patient]'s [candidate_name] with [src]."))
...
/obj/item/shears/suicide_act(mob/living/carbon/user)
	...
	for(var/obj/item/bodypart/thing in user.bodyparts)
		if(thing.body_zone == BODY_ZONE_CHEST)
			continue
