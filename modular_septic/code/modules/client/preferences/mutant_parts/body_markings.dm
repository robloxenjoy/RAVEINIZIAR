...
/datum/preference/choiced/full_body_markings
	category = PREFERENCE_CATEGORY_FEATURES
	savefile_identifier = PREFERENCE_CHARACTER
	savefile_key = "feature_body_markings"
	priority = PREFERENCE_PRIORITY_MUTANT_PART
	main_feature_name = "Full-body markings"
	relevant_mutant_bodypart = "body_markings"
	can_randomize = FALSE
	should_generate_icons = FALSE

/datum/preference/choiced/full_body_markings/create_informed_default_value(datum/preferences/preferences)
	. = ..()
	...
	if(species.default_mutant_bodyparts[relevant_mutant_bodypart])
		if(species.default_mutant_bodyparts[relevant_mutant_bodypart] == ACC_RANDOM)
			qdel(species)
			return pick(GLOB.sprite_accessories[relevant_mutant_bodypart])
		qdel(species)
		return species.default_mutant_bodyparts[relevant_mutant_bodypart]
	return "None"

/datum/preference/choiced/full_body_markings/init_possible_values()
	return assoc_to_keys(GLOB.sprite_accessories[relevant_mutant_bodypart])

/datum/preference/choiced/full_body_markings/apply_to_human(mob/living/carbon/human/target, value)
...
/datum/preference/tri_color/full_body_markings/create_informed_default_value(datum/preferences/preferences)
	var/part_type = preferences.read_preference(/datum/preference/choiced/ears)
	if(part_type)
...
/datum/preference/tri_color/full_body_markings/apply_to_human(mob/living/carbon/human/target, value)
	if(!target.dna.mutant_bodyparts[relevant_mutant_bodypart])
		target.dna.mutant_bodyparts[relevant_mutant_bodypart] = list(MUTANT_INDEX_NAME = "None", \
												MUTANT_INDEX_COLOR = list("FFFFFF", "FFFFFF", "FFFFFF"))
