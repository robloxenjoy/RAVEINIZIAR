/mob/living/carbon/human/Login()
	...
	print_msg += "<span class='infoplain'><div class='infobox'><span class='danger'><EM>As you snap back to consciousness, you recall people messing with your stuff...</EM></span>"
	...
	for(var/list/iter_theft as anything in afk_thefts)
		...
		if(time_since > AFK_THEFT_FORGET_DETAILS_TIME)
			print_msg += "\n[span_danger("<b>Someone [theft_message], but it was at least [DisplayTimeText(AFK_THEFT_FORGET_DETAILS_TIME)] ago.</b>")]"
		else
			print_msg += "\n[span_danger("<b>[thief_name] [theft_message] roughly [DisplayTimeText(time_since, 10)] ago.</b>")]"

	if(LAZYLEN(afk_thefts) >= AFK_THEFT_MAX_MESSAGES)
		print_msg += span_warning("\nThere may have been more, but that's all you can remember...")
	print_msg += "</div></span>" //div infobo

	to_chat(src, jointext(print_msg, ""))
	LAZYNULL(afk_thefts)
