/mob/living/carbon/human/check_self_for_injuries(detailed = FALSE)
	if(stat < UNCONSCIOUS)
		visible_message("<span class='notice'><b>[src]</b> examines himself.</span>", \
						"<span class='notice'><b>I examine myself.</b></span>")

	var/list/return_text = list("<span class='infoplain'><div class='infobox'>")
	return_text += "<span class='notice'><EM>Let's find out how I'm doing...</EM></span>"
	if(stat < DEAD)
		return_text += "\n<span class='notice'>I'm still alive.</span>"
		return_text += "\n<span class='notice'>I can carry up to <b>[CEILING(maximum_carry_weight, 1)]kg</b>.</span>"
		return_text += "\n<span class='notice'>Basic Lift: <b>[CEILING(maximum_carry_weight/10, 1)]kg</b>.</span>"
		return_text += "\n<span class='notice'>Weight: [encumbrance_text()] (<b>[CEILING(carry_weight, 1)]kg</b>)</span>"
		if(stat >= UNCONSCIOUS)
			return_text += "\n<span class='notice'>I'm [HAS_TRAIT(src, TRAIT_TRYINGTOSLEEP) ? "sleeping" : "uncoscious"].</span>"
		...
		if(undergoing_septic_shock())
			return_text += "\n<span class='necrosis'>I'm under septic shock!</span>"
		if(HAS_TRAIT(src, TRAIT_DEATHS_DOOR))
			return_text += "\n<span class='flashingdanger'>I'm getting ready to die!</span>"
	...
	for(var/zone in check_bodyparts)
		...
		if(LB.is_fractured())
			status += "<span class='boned'><b>FRACTURED</b></span>"
		else if(LB.is_dislocated())
			status += "<span class='boned'><b>DISLOCATED</b></span>"
		...
		if(LB.is_dead())
			...
			status += "<span class='infection'>PUS</span>"
