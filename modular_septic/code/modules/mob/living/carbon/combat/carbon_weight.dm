...
/mob/living/carbon/proc/update_carry_weight()
	. = 0
	var/list/inventory_items = list(back, wear_mask, wear_neck, head, gloves, shoes, glasses, pants, oversuit, wrists)
...
/mob/living/carbon/proc/encumbrance_text()
	switch(encumbrance)
		if(ENCUMBRANCE_EXTREME)
			return span_flashinguserdanger(span_big("ТЯЖЕЛО БЛЯТЬ!"))
		if(ENCUMBRANCE_HEAVY)
			return span_userdanger("Тяжеловато.")
		if(ENCUMBRANCE_MEDIUM)
			return span_boldnotice("Ну средне.")
		if(ENCUMBRANCE_LIGHT)
			return span_notice("Слегонца.")
		if(ENCUMBRANCE_NONE)
			return span_tinynotice("Ничего.")
