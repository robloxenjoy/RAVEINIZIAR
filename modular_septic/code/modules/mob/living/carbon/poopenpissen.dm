/mob/living/carbon/proc/shit(intentional = FALSE)
	...
	if(!LAZYLEN(intestines))
		if(intentional)
			to_chat(src, span_warning("I have no guts."))
		return
	if(intestinal_efficiency < ORGAN_FAILING_EFFICIENCY)
		if(intentional)
			to_chat(src, span_userdanger("My guts are spasming trying to shit something!"))
		return
	...
	if(collective_shit_amount < 10)
		if(intentional)
			to_chat(src, span_warning("I don't need to shit right now."))
		return
	...
	if(toiler)
		//mess gone
		qdel(stool)
		visible_message(span_notice("<b>[src]</b> shits in [toiler]."), \
					span_notice("I shit in [toiler]."))
	else
		visible_message(span_notice("<b>[src]</b> shits on [loc]."), \
					span_notice("I shit on [loc]."))
...
/mob/living/carbon/proc/piss(intentional = FALSE)
	...
	if(collective_piss_amount < 15)
		if(intentional)
			to_chat(src, span_warning("I don't need to piss right now."))
		return
	...
	if(urinel)
		visible_message(span_notice("<b>[src]</b> pisses in [urinel]."), \
					span_notice("I piss in [urinel]."))
		for(var/obj/item/organ/bladder/bladder as anything in bladders)
		...
		visible_message(span_notice("<b>[src]</b> pisses in [toiler]."), \
					span_notice("I piss in [toiler]."))
		for(var/obj/item/organ/bladder/bladder as anything in bladders)
			var/amount = min(40, bladder.reagents.get_reagent_amount(/datum/reagent/consumable/piss) - bladder.food_reagents[/datum/reagent/consumable/piss])
			bladder.reagents.remove_reagent(/datum/reagent/consumable/piss, amount)
	else
