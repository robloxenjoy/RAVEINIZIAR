	...
	if(fraggot)
		for(var/mob/living/carbon/human/M in range(7, src))
			if(M != src && (src in view(M)))
				to_chat(M, span_achievementrare("I have seen a fraggot death! So good!"))
				SEND_SIGNAL(M, COMSIG_ADD_MOOD_EVENT, "[our_fraggot.real_name]", /datum/mood_event/fraggot/killed)
