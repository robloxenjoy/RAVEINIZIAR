		...
		*/
		...
		if(BODY_ZONE_PRECISE_L_EYE)
			...
			if(!user.canUseTopic(src, BE_CLOSE, FALSE, NO_TK))
				return
			if(new_eye_color)
				H.left_eye_color = sanitize_hexcolor(new_eye_color, 6, FALSE)
				H.dna.update_ui_block(DNA_LEFT_EYE_COLOR_BLOCK)
				H.update_body()
		if(BODY_ZONE_PRECISE_R_EYE)
			var/new_eye_color = input(H, "Choose your right eye color", "Eye Color","#"+H.right_eye_color) as color|null
			if(!user.canUseTopic(src, BE_CLOSE, FALSE, NO_TK))
				return
			if(new_eye_color)
