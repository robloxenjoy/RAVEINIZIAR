...
/turf
	...
	var/mood_bonus_turf = 0
	var/mood_turf_mes = "<span class='nicegreen'>This floor is pretty nice!</span>\n"
	var/mood_turf_trait
...
/turf/proc/zImpact(atom/movable/A, levels = 1, turf/prev_turf)
	...
	if(prev_turf && !(flags & FALL_NO_MESSAGE))
		prev_turf.visible_message(span_danger("[mov_name] падает в [prev_turf]!"))
	if(flags & FALL_INTERCEPTED)
		return
	if(zFall(A, levels + 1))
		return FALSE
	A.visible_message(span_danger("[A] врезается в [src]!"))
...
/turf/handle_fall(mob/faller)
	...
	faller.visible_message("<span class='alert'><b>[faller]</b> collapses!</span>", \
							"<span class='userdanger'>I collapse!</span>", \
							"<span class='alert'>I hear a thud!</span>", \
