...
/datum/component/mood/proc/check_turf_mood(datum/source, turf/A)
	SIGNAL_HANDLER

	update_beauty(A)
	if(A.mood_bonus_turf && (!A.mood_turf_trait || HAS_TRAIT(source, A.mood_turf_trait)))
		add_event(null, "turf", /datum/mood_event/turf, A.mood_bonus_turf, A.mood_turf_mes)
	else
		clear_event(null, "turf")
