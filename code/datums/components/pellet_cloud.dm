...
/datum/component/pellet_cloud/Initialize(projectile_type=/obj/item/shrapnel, magnitude=5)
	if(!isammocasing(parent) && !isgrenade(parent) && !islandmine(parent) && !issupplypod(parent) && !iscrazymine(parent))
	...
	else if(isgrenade(parent) || islandmine(parent) || issupplypod(parent) || iscrazymine(parent))
		radius = magnitude
...
/datum/component/pellet_cloud/RegisterWithParent()
	...
	else if(iscrazymine(parent))
		RegisterSignal(parent, COMSIG_CRAZYMINE_TRIGGERED, PROC_REF(create_blast_pellets))
	else if(issupplypod(parent))
		RegisterSignal(parent, COMSIG_SUPPLYPOD_LANDED, PROC_REF(create_blast_pellets))

/datum/component/pellet_cloud/UnregisterFromParent()
	UnregisterSignal(parent, list(COMSIG_PARENT_PREQDELETED, COMSIG_PELLET_CLOUD_INIT, COMSIG_GRENADE_DETONATE, COMSIG_GRENADE_ARMED, COMSIG_MOVABLE_MOVED, COMSIG_CRAZYMINE_TRIGGERED, COMSIG_MINE_TRIGGERED, COMSIG_ITEM_DROPPED))
...
/datum/component/pellet_cloud/proc/create_blast_pellets(obj/O, mob/living/triggerer)
	...
	if(isgrenade(parent)) // handle_martyrs can reduce the radius and thus the number of pellets we produce if someone dives on top of a frag grenade
		...
		INVOKE_ASYNC(src, PROC_REF(handle_martyrs), triggerer)
	else if(islandmine(parent))
...
/datum/component/pellet_cloud/proc/pew(atom/target, landmine_victim)
	...
	//SEPTIC EDIT BEGIN
	RegisterSignal(P, COMSIG_PELLET_CLOUD_EMBEDDED, PROC_REF(projectile_embedded))
	RegisterSignal(P, COMSIG_PELLET_CLOUD_STOPPED_BY_ARMOR, PROC_REF(projectile_stopped_by_armor))
	RegisterSignal(P, COMSIG_PELLET_CLOUD_WENT_THROUGH, PROC_REF(projectile_went_through))
	//SEPTIC EDIT END
