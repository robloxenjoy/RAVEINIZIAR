...
/datum/component/gunpoint/proc/async_trigger_reaction()
	...
	if(!weapon.can_shoot() || !weapon.can_trigger_gun(shooter) || (weapon.weapon_weight == WEAPON_HEAVY && !SEND_SIGNAL(weapon, COMSIG_TWOHANDED_WIELD_CHECK) && (GET_MOB_ATTRIBUTE_VALUE(shooter, STAT_STRENGTH) < 20)))
