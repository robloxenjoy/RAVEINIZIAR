...
/datum/element/climbable/proc/on_examine(atom/source, mob/user, list/examine_texts)
	SIGNAL_HANDLER
	examine_texts += span_notice("[source] выглядит так, что на него можно залезть.")
...
/datum/element/climbable/proc/mousedrop_receive(atom/receiver, atom/movable/dropped_atom, mob/user, params)
	SIGNAL_HANDLER
	if(user == dropped_atom && isliving(dropped_atom))
		...
		if(living_target.mobility_flags & MOBILITY_MOVE)
			INVOKE_ASYNC(src, .proc/climb_structure, receiver, living_target, params)
			return
