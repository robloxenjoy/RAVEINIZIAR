/obj/item/clothing/under
	...
	body_parts_covered = VITALS|CHEST|GROIN|LEGS|ARMS
	...
	if(adjusted)
		...
		if(!alt_covers_chest)
			...
			if(!LAZYLEN(damage_by_parts))
				return adjusted
			for(var/zone in list(BODY_ZONE_CHEST, BODY_ZONE_PRECISE_VITALS, BODY_ZONE_L_ARM, BODY_ZONE_R_ARM)) // ugly check to make sure we don't reenable protection on a disabled part
