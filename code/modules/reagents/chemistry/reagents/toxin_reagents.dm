...
/datum/reagent/toxin/badwater/shallow/expose_atom(atom/exposed_atom, reac_volume)
	. = ..()
	if(!.)
		exposed_atom.adjust_germ_level(-reac_volume * SANITIZATION_PER_UNIT_WATER)

/datum/reagent/toxin/badwater/shallow/on_mob_life(mob/living/carbon/M, delta_time, times_fired)
	if(ishuman(M))
