	...
	*/
	...
	if(RE || LE)
		if(!active)
			...
			RE?.flash_protect = FLASH_PROTECTION_SENSITIVE //Adjust the user's eyes' flash protection
			LE?.sight_flags |= SEE_MOBS | SEE_OBJS | SEE_TURFS //Add sight flags to the user's eyes
			LE?.flash_protect = FLASH_PROTECTION_SENSITIVE //Adjust the user's eyes' flash protection
			to_chat(user, "We adjust our eyes to sense prey through walls.")
			active = TRUE //Defined in code/modules/spells/spell.dm
		else
			RE?.sight_flags ^= ~(SEE_MOBS | SEE_OBJS | SEE_TURFS) //Remove sight flags from the user's eyes
			RE?.flash_protect = FLASH_PROTECTION_WELDER //Adjust the user's eyes' flash protection
			LE?.sight_flags &= ~(SEE_MOBS | SEE_OBJS | SEE_TURFS) //Remove sight flags from the user's eyes
			LE?.flash_protect = FLASH_PROTECTION_WELDER //Adjust the user's eyes' flash protection
			to_chat(user, "We adjust our eyes to protect them from bright lights.")
			active = FALSE
		user.update_sight()
	//SEPTIC EDIT END
	...
	*/
	...
	if(RE || LE)
		if(active)
			...
			LE?.sight_flags ^= SEE_MOBS | SEE_OBJS | SEE_TURFS
		else
