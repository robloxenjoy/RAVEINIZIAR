...
/mob/living/carbon/human/attack_larva(mob/living/carbon/alien/larva/L)
	...
	if(!damage)
		return
	if(check_shields(L, damage, attacking_flags = BLOCK_FLAG_UNARMED) & COMPONENT_HIT_REACTION_BLOCK)
		return FALSE
	if(stat != DEAD)
...
/mob/living/carbon/human/attack_basic_mob(mob/living/basic/user, list/modifiers)
	...
	if(check_shields(user, damage, attacking_flags = BLOCK_FLAG_UNARMED) & COMPONENT_HIT_REACTION_BLOCK)
		return FALSE
...
//160 max knockdown for EXPLODE_LIGHT
	...
	switch (severity)
		...
		if (EXPLODE_HEAVY)
			...
			burn_loss = 70
			if(bomb_armor)
				brute_loss = 40*(2 - round(bomb_armor*0.01, 0.05))
