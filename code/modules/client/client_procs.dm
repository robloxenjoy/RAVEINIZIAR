...
*/
		if (!stealth_admin)
			deadchat_broadcast(" рекконектнулся.", "<b>[mob][mob.get_realname_string()]</b>", follow_target = mob, turf_target = get_turf(mob), message_type = DEADCHAT_LOGIN_LOGOUT, admin_only=!announce_leave)
...
/client/proc/check_panel_loaded()
	if(statbrowser_ready)
		return
	src << browse(file('html/statbrowser.html'), "window=statbrowser")
	addtimer(CALLBACK(src, PROC_REF(check_panel_loaded_twice)), 10 SECONDS)
