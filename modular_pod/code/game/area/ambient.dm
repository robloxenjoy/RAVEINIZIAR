...
/mob/proc/handle_sounds()
	...
	if(!ambient_music || (!ambient_music.transition && (world.time >= ambient_music.last_time + ambient_music.real_cooldown)))
		...
		if(A.ambient_music && (!music || (music && music.volume <= 0)))
			...
			////////////////////////
			ambient_music.Set_Sound(SSchannels.get_reserved_channel(15), 10, 0, -1)
			src << ambient_music

	if(!ambient_environment || world.time >= ambient_environment.last_time + ambient_environment.real_cooldown)

		if(A.ambient_environment && !isblowout)

			if(A.ambient_environment_night && (SSsunlighting.current_step == STEP_EVENING || SSsunlighting.current_step == STEP_NIGHT))
			...
			if(ambient_environment)
				...
				////////////////////////
				ambient_environment.Set_Sound(SSchannels.get_reserved_channel(16), rand(25, 60), rand(-100, 100), A.environment)
	...
	if(!ambient_background || (!ambient_background.transition && world.time >= ambient_background.last_time + ambient_background.real_cooldown))
		...
		if(A.ambient_background)
			...
			if(A.ambient_background[SSsunlighting.current_step])
				ambient_background = sound(file = A.ambient_background[SSsunlighting.current_step])
				ambient_background.real_cooldown = A.ambient_background_cooldown[SSsunlighting.current_step]

			if(ambient_background)
				////////////////////////
				ambient_background.last_time = world.time
				////////////////////////
				ambient_background.Set_Sound(SSchannels.get_reserved_channel(17), 35, 0, A.environment)
...
/*
	if(src.psyloss >= 25 && (!ambient_psy || (world.time >= ambient_psy.last_time + ambient_psy.real_cooldown)))
		...
		////////////////////////
		ambient_psy.Set_Sound(SSchannels.get_reserved_channel(18), 60*(psyloss/200), 0, -1)
		src << ambient_psy
*/
